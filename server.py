...
def main():
    ...
    log = input("Write this session on a file? (y/n) ").strip().lower()

    if(log == 'n'):
        log_check = False
        print(f"{Fore.CYAN}[+]{Fore.CYAN}{Fore.RESET}{Fore.YELLOW} Not logging{Fore.YELLOW}{Fore.RESET}")
    else :
        log_check = True
        print(f"{Fore.CYAN}[+]{Fore.CYAN}{Fore.RESET}{Fore.YELLOW} Log True{Fore.YELLOW}{Fore.RESET}")
    ...
    accept_thread = threading.Thread(target=sm.accept_connections,args=(server_socket,cipher,user,room_password,log_check))
    send_thread = threading.Thread(target=sm.send_messages,args=(user,cipher,log_check))
